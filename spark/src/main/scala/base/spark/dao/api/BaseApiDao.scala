package base.spark.dao.api

import akka.actor.{Actor, ActorRef, Props, Terminated}
import base.spark.common.Constants
import base.spark.common.config.ApiConfigTrait
import base.spark.dao.hive.impl.HiveDaoImpl
import base.spark.util.{ActorUtil, DataFrameUtil, DateUtil, HttpRequest, HttpUtil}
...
abstract class BaseApiDao(val db: String, val tableName: String, val gradient: Int, val tableType: String = "hive", val pageSize: Int = 1000) {
  ...
  protected val httpUtil = new HttpUtil()
  ...
  val client: CloseableHttpClient = HttpClients.custom().setConnectionManager(manager).build()
