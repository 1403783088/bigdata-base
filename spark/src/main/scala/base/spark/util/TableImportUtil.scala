...
import base.spark.common.config.TableConfig
import base.spark.dao.hive.impl.HiveDaoImpl
import base.spark.dao.kudu.impl.KuduDaoImpl
import base.spark.dao.mysql.impl.MySqlDaoImpl
import base.spark.dao.oracle.impl.OracleDaoImpl
import base.spark.dao.sqlserver.impl.SqlServerDaoImpl
    ...
    if(begin.equals("")  || finish.equals("")){
        ...
        case "T0" =>
          source_delta_fields.zip(target_delta_fields).foreach{
            case (source_delta_field, target_delta_field) =>
              ...
              source_type match {
                case "oracle" =>
                  delta_condition.append(s" or ($source_delta_field > to_date('$lastMaxValue', 'yyyy-mm-dd hh24:mi:ss'))")
                case _ =>
                  delta_condition.append(s" or ($source_delta_field > '$lastMaxValue')")
              }
          }
          delta_condition.substring(3)
        case "T1" =>
          source_delta_fields.foreach(source_delta_field => {
            source_type match {
              case "oracle" =>
                delta_condition
                  .append(s" or ($source_delta_field >= to_date('$yesterdayTime', 'yyyy-mm-dd hh24:mi:ss')" +
                    s" and $source_delta_field < to_date('$todayTime', 'yyyy-mm-dd hh24:mi:ss'))")
              case _ =>
                delta_condition
                  .append(s" or ($source_delta_field >= '$yesterdayTime' and $source_delta_field < '$todayTime')")
            }
          })
          delta_condition.substring(3)
      }
      ...
      source_delta_fields.foreach(source_delta_field => {
        source_type match {
          case "oracle" =>
            delta_condition
              .append(s" or ($source_delta_field >= to_date('$begin', 'yyyy-mm-dd hh24:mi:ss')" +
                s" and $source_delta_field < to_date('$finish', 'yyyy-mm-dd hh24:mi:ss'))")
          case _ =>
            delta_condition
              .append(s" or ($source_delta_field >= '$begin' and $source_delta_field < '$finish')")
        }
      })
      delta_condition.substring(3)
    }
  ...
  private def saveData(target_type: String,
    ...
    target_type match {
      case "hive" =>
        update_type match {
          case "full" =>
            new HiveDaoImpl(spark).write(target_db, target_table_name, DataFrameUtil.cleanDataFrame(data, spark = spark), SaveMode.Overwrite)
          case "delta" =>
      ...
      case "kudu" =>
        update_type match {
          case "full" =>
            new KuduDaoImpl(spark).write(target_db, target_table_name, DataFrameUtil.cleanDataFrame(data, spark = spark), "overwrite")
          case "delta" =>
    ...
    val (metadata_table, tb_table, db_column, tb_column, col_column, comment_column) =
      TableConfig.source_type match {
        case "oracle" => ("all_col_comments",
          ...
          "COLUMN_NAME",
          "COMMENTS")
        case "mysql" => ("INFORMATION_SCHEMA.COLUMNS",
          "INFORMATION_SCHEMA.TABLES",
          "TABLE_SCHEMA",
          "TABLE_NAME",
          "COLUMN_NAME",
          "COLUMN_COMMENT")
        case "sqlserver" => ("(SELECT D.TABLE_CATALOG as db, A.name AS table_name, B.name AS column_name, convert(varchar(255), C.value) AS column_comment FROM sys.tables A INNER JOIN sys.columns B ON B.object_id = A.object_id LEFT JOIN sys.extended_properties C ON C.major_id = B.object_id AND C.minor_id = B.column_id inner join INFORMATION_SCHEMA.COLUMNS D on A.name = D.TABLE_NAME) a",
          "INFORMATION_SCHEMA.TABLES",
          "db",
          "table_name",
          "column_name",
